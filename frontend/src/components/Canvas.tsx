...
import { Link2Off, MousePointer } from "lucide-react";
...
const Canvas = forwardRef<HTMLDivElement, CanvasProps>(({
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      panZoomRef.current = panzoom(canvasRef.current, {
        ...
        bounds: true,
        boundsPadding: 0.1,
        // This is the critical change - filter events to not affect box interaction
        beforeWheel: function (e) {
          // Allow wheel events regardless of where they occur
          return false; // false means don't prevent default
        },
        beforeMouseDown: function (e) {
          // Check if click was on a box or interactive element
          const target = e.target as HTMLElement;

          // If the click is inside a CodeBox or on a connection point or on a button, let the box handle it
          if (
            target.closest(".codebox") ||
            target.closest(".box-header") ||
            target.closest(".connection-point") ||
            target.closest(".resize-handle") ||
            target.classList.contains("resize-handle") ||
            target.classList.contains("connection-point")
          ) {
            return true; // prevent panzoom from handling this event
          }

          // If in create mode, allow canvas to handle clicks for box creation
          if (interactionMode === "create") {
            return true; // prevent panzoom
          }

          return false; // allow panzoom to handle this event
        },
      });
  ...
  useEffect(() => {
    if (panZoomRef.current) {
      // Always enable panning - we'll handle drag events on boxes separately
  ...
  const handleLinkStart = (boxId: string) => {
    if ((interactionMode === "link" || interactionMode === "edit")) {
  ...
  const handleLinkEnd = (boxId: string) => {
    if (linking.isActive && linking.sourceBoxId) {
      ...
      if (
        LinkingManager.isCircularDependency(linking.sourceBoxId, boxId, arrows)
      ) {
  ...
  return (
    <div className="flex-1 relative overflow-hidden bg-gray-50">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(rgba(150, 150, 150, 0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(150, 150, 150, 0.1) 1px, transparent 1px)",
          ...
          {boxes.map((box) => (
            <CodeBox
              ...
              onResize={(width, height) =>
                handleBoxResize(box.id, width, height)}
              ...
              isLinkTarget={linking.isActive && linking.sourceBoxId !== null &&
                linking.sourceBoxId !== box.id}
              onLinkStart={() => handleLinkStart(box.id)}
              onLinkEnd={() => handleLinkEnd(box.id)}
              scale={currentScale} // Pass current scale to CodeBox
            />
          ))}
