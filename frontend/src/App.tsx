    ...
    if (!loading && boxes.length > 0) {
      ...
      saveTimeoutRef.current = window.setTimeout(async () => {
        try {
          ...
          setError('Failed to save workspace. Your changes may not be persisted.');
        } finally {
          setSaving(false);
        }
    ...
    switch (interactionMode) {
      ...
      case 'link':
        return (
          <div className="flex items-center gap-2 text-sm text-gray-600 bg-blue-50/50 px-3 py-1.5 rounded">
            <Link size={16} />
            <span>Click on connection points to create links between boxes</span>
          </div>
        );
      default:
        return null;
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <div className="flex justify-between items-center p-2.5 bg-white border-b border-gray-200 shadow-sm z-10">
        <ModeSelector activeMode={interactionMode} onModeChange={handleModeChange} />
